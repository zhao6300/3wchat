...
class PromptFactory:
    ...
    def create_strategy_prompt(self, snapshot: SituationalSnapshot, history: str, openai_tools: List[Dict]) -> str:
        ...
        **第一部分：动作执行类策略 (优先判断)**
        ...
        1.  **`{ActionStrategy.TOOL_USE.value}` (工具调用):**
            *   **何时使用:** 当用户的请求可以直接通过调用一个或多个**具体的功能性工具**来完成时。这适用于执行动作（如计算、查财报），而非查找宽泛信息。
            *   **思考过程:** “这个任务有现成的工具可以完美解决。”
            *   **示例:** "计算 100 * (5+3)", "阿里巴巴的收入是多少？"。

        2.  **`{ActionStrategy.RAG_QUERY.value}` (检索增强生成):**
            *   **何时使用:** 当用户的问题很可能可以从一个**特定的、内部的、私有的知识库**中找到答案时。这适用于查询事实、数据或已归档的信息。
            *   **思考过程:** “这个问题看起来像是在查阅一份内部文档或数据库。”
            *   **示例:** "查一下腾讯的简介" (如果知识库中有公司资料)。

        3.  **`{ActionStrategy.SEARCH_AGENT_DELEGATION.value}` (委托搜索代理):**
            *   **何时使用:** 当用户需要**开放域的、公共的、最新的信息**时。这适用于定义、概念解释、新闻、教程或任何通常需要用搜索引擎才能找到的内容。
            *   **思考过程:** “这个问题很宽泛，或者需要最新的信息，本地知识库里肯定没有。我需要让一个专门的搜索代理去网上查。”
            *   **示例:** "什么是 LangGraph？", "RAG 和 Agent Memory 有什么区别？"。

        ---
        **第二部分：回应生成类策略 (若无动作可执行，则选择其一)**

        4.  **`{ActionStrategy.DIRECT_ANSWER.value}` (直接解答):**
            *   **何时使用:** 当你可以完全依靠自己的内置知识库，无需外部信息或工具，就能提供一个**不含疑问、不含多个选项的单一、明确的回答**时。
            *   **思考过程:** “这个问题我可以直接给出一个确切答案。”
            *   **示例:** "你好", "写一首关于春天的诗"。

        5.  **`{ActionStrategy.CLARIFY_ASK.value}` (澄清反问):**
            *   **何时使用:** 当用户的请求不够清晰，需要提出**少量（3个及以下）**的后续问题来明确其意图时。回答中不应包含对可能答案的推测性回应。
            *   **思考过程:** “我需要更多信息才能准确回答，问几个问题就好了。”
            *   **示例:** 用户问“推荐一家餐厅”，AI反问“请问您喜欢什么菜系？预算大概多少？”

        6.  **`{ActionStrategy.INTERROGATE.value}` (深入追问):**
            *   **何时使用:** 当用户的请求非常模糊或宽泛，需要提出**大量（超过3个）**的后续问题进行深入探讨时。这通常用于头脑风暴或复杂问题的初步探索阶段。
            *   **思考过程:** “用户的问题太开放了，我需要通过一连串问题来帮助他理清思路。”
            *   **示例:** 用户问“我该如何规划我的人生？”，AI反问“这需要从几个方面来看，比如你的兴趣是什么？你的长期职业目标是？你对生活方式有何期待？...”

        7.  **`{ActionStrategy.HEDGING.value}` (对冲回答):**
            *   **何时使用:** 当问题没有单一的正确答案，或答案取决于多种不确定的条件时。你的回答不给出一个明确的立场，而是提供**多种可能性、观点或选项**，并可能讨论它们各自成立的条件。
            *   **思考过程:** “这个问题太复杂/主观了，没有标准答案。我应该把各种可能性都列出来，让用户自己判断。”
            *   **示例:** “未来AI会取代人类吗？”，回答可以讨论“强人工智能”和“弱人工智能”的不同观点，以及技术、伦理等多种因素。

        8.  **`{ActionStrategy.REFUSE.value}` (拒绝回答):**
            *   **何时使用:** 当用户的请求涉及不安全、不道德、违法或你无法回答的内容时，应**明确或委婉地拒绝**。可以说明拒绝的原因，但不要提供问题的替代解决方案。
            *   **思考过程:** “这个请求我不能满足。”
            *   **示例:** “如何制造炸弹？”，AI回答“抱歉，我无法提供关于制造危险物品的信息。”
        ---
        ...
        *   **如果 `strategy` 是 `"{ActionStrategy.CLARIFY_ASK.value}"` 或 `"{ActionStrategy.INTERROGATE.value}"`：**
            *   用一个或多个向用户提出的问题填充 `clarification_question` 字段。
            *   所有其他特定于行动的字段**必须**为空 (`""` 或 `[]`)。
        *   **如果 `strategy` 是 `"{ActionStrategy.DIRECT_ANSWER.value}"`, `"{ActionStrategy.HEDGING.value}"`, `"{ActionStrategy.REFUSE.value}"` 或任何其他非动作、非提问的策略：**
